import { response,findAdminUser,makeSession,audit,client } from './_admin.js';

export default async function handler(request){
  if(request.method!=='POST') return response(405,{ok:false,error:'POST required'});
  try{
    const body=await request.json().catch(()=>({}));
    const user=findAdminUser(body.username,body.password);
    ...
    try{
      ...
      if(url&&key){
        const s=client(url,key);
        await s.from('admin_audit').insert({admin_phone:'',actor_name:user.display_name,actor_email:user.username+'@excelsior26.local',action:'admin_login',meta:{username:user.username,role:user.role}});
      }
    }catch{}
