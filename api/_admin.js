...
export async function requireAdmin(request,minimum='admin'){
  ...
  const admin={...user,email:first?.email||user.username+'@excelsior26.local',phone:first?.phone||'',id:first?.id||null,enabled:true};return{admin,supabase:s,role:user.role};
}
